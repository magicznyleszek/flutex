 ...
 * `data-mantine-color-scheme` once React has mounted — several hundred milliseconds of the wrong background —
 * so index.html loads this as a blocking classic script.
 *
 * The device decides, unless the toggle has overridden it earlier in this session. That is `COLOR_SCHEME_KEY`
 * of colorScheme.ts, spelled out again because a classic script cannot import.
 */
...
/**
 * The attribute and nothing else. Mantine's stylesheet already ties `color-scheme` to it, and an inline
 * `style.colorScheme` here would win over that — leaving the scrollbars on the old scheme.
 */
